                        ...
                        <div class="mb-3">
                            ...
                            <select name="genre_id" id="genre_id" class="form-select @error('genre_id') is-invalid @enderror">
                                ...
                                @foreach($genres as $genre)
                                    <option value="{{ $genre->id }}" {{ old('genre_id', $movie->genre_id) == $genre->id ? 'selected' : '' }}>{{ $genre->gender }}</option>
                                @endforeach
                            </select>
                            @error('genre_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="format" id="format" class="form-select @error('format') is-invalid @enderror">
                                ...
                                @foreach($formats as $value => $label)
                                    <option value="{{ $value }}" {{ old('format', $movie->format) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('format')
                                <div class="invalid-feedback">{{ $message }}</div>
